...
project_data = ProjectSiteData(
    ...
    bodyContent = [
        ...
        BodyContent("UnityEvents Trigger Components", r"July 28, 2026",
            ...
            <ul>
              <li>A timer which invokes after a time limit, that can be set per-component or per-call</li>
              <li>A one-time environmental trigger box</li>
              <li>A toggle trigger which alternates between two sets of events each time it is invoked, or can be set directly to invoke a particular state</li>
              <li>An environmental trigger which invokes when interacted with</li>
              <li>A trigger which invokes on Start()</li>
              <li>A trigger which goes off periodically, with adjustable randomness</li>
            </ul>
